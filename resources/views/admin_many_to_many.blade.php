    ...
    <script>
        $(document).ready( function () {
            ...
            var table = $('#adminTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin_many_to_many.index') }}",
                    type: 'GET',
                    data: function (d) {
            ...
            $("#member_id").select2({
                theme: 'bootstrap-5',
                ajax: { 
                    url: "{{ route('admin_many_to_many.member') }}",
                    type: "post",
                    dataType: 'json',
                    delay: 250,
            ...
            $(document).on('submit', '#formCreate', function(event){  
                ...
                $.ajax({
                    url: "{{ route('admin_many_to_many.create') }}",
                    cache: false,  
                    method:'POST',  
                    data:  $(this).serialize(),
            ...
            $(document).on("click", ".editBrand", function () {
                ...
                $.ajax({
                    type:"POST",
                    url: "{{ url('/admin_many_to_many/edit') }}"+'/'+id,
                    data: { id: id },
                    dataType: 'json',
            ...
            $(document).on('click', '#buttonUpdate', function(event){  
                ...
                var id = $('#id').val();  
                $.ajax({
                    url: "{{ url('/admin_many_to_many/update') }}"+'/'+id,
                    cache: false,
                    method:"POST",
            ...
            $(document).on('click', '.delete', function(){  
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url('/admin_many_to_many/delete') }}"+'/'+id,
                            cache: false,
                            method:"DELETE",  
                            data: { id: id },
                            success: function(data){
